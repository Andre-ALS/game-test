...
const OUT = path.join(ROOT, "src/assets/sprites/equipments-v2.png");
...
function buildSheet(stations) {
  ...
  for (let i = 0; i < stations.length; i++) {
    const bottomSprite = stations[i];
    const oy = i * (cell + GAP);

    const bottom = placeOnGround(bottomSprite, cell);

    // Trash bin: single frame only
    if (i === stations.length - 1) {
      blit(sheet, bottom, 0, oy);
      continue;
    }

    // Left = front faces left (rotate CW so former bottom edge becomes left,
    // then plant base on ground again).
    const left = placeOnGround(rotate90(bottomSprite, "cw"), cell);
    const right = placeOnGround(rotate90(bottomSprite, "ccw"), cell);

    blit(sheet, left, 0, oy);
    blit(sheet, bottom, cell + GAP, oy);
    blit(sheet, right, (cell + GAP) * 2, oy);
  }
